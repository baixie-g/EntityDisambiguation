...
def test_linear_normalization():
    ...
    print("📊 线性归一化效果:")
    print("-" * 60)
    print(f"{'测试用例':<25} {'原始得分':<12} {'线性归一化':<15} {'说明'}")
    print("-" * 60)
...
def test_weighted_scores():
    ...
    print(f"\n🧮 修正后加权得分计算:")
    print("  加权得分 = BGE×0.4 + CrossEncoder(线性归一化)×0.3 + Fuzz×0.2 + Levenshtein×0.1")
    
    print(f"\n{'匹配情况':<12} {'BGE':<8} {'Cross(原始)':<12} {'Cross(归一化)':<12} {'Fuzz':<8} {'Levenshtein':<12} {'加权得分':<12} {'状态'}")
    print("-" * 100)
    
    for case in test_cases:
        # 计算线性归一化的CrossEncoder得分
...
def compare_normalization_methods():
    ...
    print("📊 Sigmoid vs 线性归一化对比:")
    print("-" * 70)
    print(f"{'匹配类型':<12} {'原始得分':<12} {'Sigmoid':<12} {'线性':<12} {'差异'}")
    print("-" * 70)
    
    for match_type, original_score in test_scores:
        # Sigmoid归一化
        sigmoid_normalized = 1 / (1 + np.exp(-original_score))
        
        # 线性归一化
        min_score = -6.5
        max_score = 7.7
        linear_normalized = (original_score - min_score) / (max_score - min_score)
        linear_normalized = max(0.0, min(1.0, linear_normalized))
        
        # 计算差异
        difference = sigmoid_normalized - linear_normalized
        
        print(f"{match_type:<12} {original_score:<12.3f} {sigmoid_normalized:<12.3f} {linear_normalized:<12.3f} {difference:<12.3f}")
...
def main():
    ...
    # 测试线性归一化效果
    normalized_scores = test_linear_normalization()
    ...
    print("✅ 线性归一化修正完成:")
    print("  1. CrossEncoder得分已使用线性函数归一化")
    ...
    print("\n📋 实施内容:")
    print("  1. 修改了normalize_crossencoder_score函数")
    print("  2. 使用线性归一化替换sigmoid归一化")
    print("  3. 更新了配置注释")
    print("  4. 验证了归一化效果")
